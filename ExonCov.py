		...
		dic_trans=make_Dic_Trans(exoncov,column)						## make dictionary for transcripts with all exons
			...
			final_list=[]
                        for exon in dic_trans[item]:
                                list=[]
                                list+=item,exon[4],exon[0],exon[1],exon[2],int(exon[2])-int(exon[1])	## Gene, chromosome, from, to, interval (too hardcoded, change?)
				list+=[int("%.0f" % float(float(int(exon[2])-int(exon[1]))*(float(exon[8]))))]  ## add number of bases covered in total for target
                                for cov in exon[(int(L_bed+3)-1):((int(L_bed+3)-1)+int(len(L_list)))]: 	## int(L_bed+3)-1)= length BED file + 3 output fields sambamaba (-1 for offset). len(list) = values as input -L option sambamba
...
def get_Longest_Transcript(transcript_files,pref_gene_dic,pref_dic,gene_dic):
	transcript=open(str(transcript_files[0]),"r").readlines()					## longest transcript needs to be determined for only 1 sample as BED files used are the same!
	dic={}
	...
	pref_dic={}
		...
		if "#" not in line:
	                ...
	                try:
	                        ...
	                        if pref_dic[splitline[1].split(".")[0]][1]==splitline[1].split(".")[0]:  ## prevent double entries because of missing version in transcript ID
	                                pass
       	                	else:
...
if __name__ == "__main__":
	...
	dic=get_Longest_Transcript(transcript_files,pref_gene_dic,pref_dic,gene_dic)	## get preferred transcript, or if not known, the longest NM or ENST transcript.
	panel_coverage={}
