...
class APITokensAdminView(CustomModelView):
    ...
    column_formatters_detail = {
        ...
        'user': lambda v, c, m, p: Markup('{} <span class="glyphicon glyphicon-user" title="Generated by me"></span>'
                                          .format(m.user))
    ...
    column_formatters = {
        'token': lambda v, c, m, p: Markup((
            '''{}...{} <a href="/admin/apitokens/details/?id={}&url=%2Fadmin%2Fapitokens%2F%3Fsort%3D0&modal=True" \
            data-target="#fa_modal_window" data-toggle="modal">see full token'''
                                            .format(m.token[:4], m.token[-4:], m.id))),
        'user': lambda v, c, m, p: Markup('{} <span class="glyphicon glyphicon-user" title="Generated by me"></span>'
                                          .format(m.user))
        if m.user_id == current_user.id else m.user
    }
