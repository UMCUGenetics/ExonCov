...
class APITokensAdminView(CustomModelView):
    ...
    details_modal = True
    create_modal = True
    column_labels = dict(application='Application name', token="API Token", user="Generated by")

    column_formatters_detail = {
        'token': lambda v, c, m, p: Markup(('''{}<br><br><div class="alert alert-warning" role="alert"><span class="
                                            glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span><span class="
                                            sr-only">Error:</span>&nbsp;Keep this token secret and do not use it for other
                                            applications</div>'''.format(m.token))),
        'user': lambda v, c, m, p: Markup('{} <span class="glyphicon glyphicon-user" title="Generated by me"></span>'.format(m.user))
        if m.user_id == current_user.id else m.user
    }

    column_formatters = {
        'token': lambda v, c, m, p: Markup(('''{}...{} <a href="/admin/apitokens/details/?
        id={}&url=%2Fadmin%2Fapitokens%2F%3Fsort%3D0&modal=True" data-target="#fa_modal_window" data-toggle="modal">
        see full token'''.format(m.token[:4], m.token[-4:], m.id))),
        'user': lambda v, c, m, p: Markup('{} <span class="glyphicon glyphicon-user" title="Generated by me"></span>'.format(m.user))
        if m.user_id == current_user.id else m.user
    }

    column_list = ["application", "token", "user", "modified_on"]
